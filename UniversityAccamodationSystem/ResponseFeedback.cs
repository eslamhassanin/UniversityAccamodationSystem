...
{
    public partial class ResponseFeedback : Form
    {
        DataTable dtb1;
        ...
        private void ResponseFeedback_Load(object sender, EventArgs e)
        {
            ShowData_Load();
        }
        public void UpdateSelectedAccept(String id)
        {
            SQLiteConnection sqlite_conn;
            sqlite_conn = CreateConnection();
            SQLiteCommand sqlite_cmd;
            sqlite_cmd = sqlite_conn.CreateCommand();
            sqlite_cmd.CommandText = "update Feedback set status=@status where StdID=@stdID";
            sqlite_cmd.Parameters.AddWithValue("@status", "Approved");
            sqlite_cmd.Parameters.AddWithValue("@stdID", id);
            int res = 0;
            try
            {
            ...
            if (res > 0)
            {
                MessageBox.Show("Request Updated Successfully.", "Message");
            }
        }


        public void UpdateSelectedReject(String id)
        {
            SQLiteConnection sqlite_conn;
            sqlite_conn = CreateConnection();
            SQLiteCommand sqlite_cmd;
            sqlite_cmd = sqlite_conn.CreateCommand();
            sqlite_cmd.CommandText = "update Feedback set status=@status where StdID=@stdID";
            sqlite_cmd.Parameters.AddWithValue("@status", "Rejected");
            sqlite_cmd.Parameters.AddWithValue("@stdID", id);
            int res = 0;
            try
            {
        ...
        {
            string StdID = "";
            if (dgvFeedback.SelectedRows.Count == 1)
            {
                foreach (DataGridViewRow row in dgvFeedback.SelectedRows)
                {
                    StdID = row.Cells[0].Value.ToString();
                    UpdateSelectedAccept(StdID);
                    ShowData_Load();
                }
            }
            else
